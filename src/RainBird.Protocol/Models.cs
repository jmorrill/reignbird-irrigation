...
public sealed record ControllerCapabilities
{
    ...
    /// is not described anywhere we have — not in the app's own resource tables, and
    ...
    ///
    /// Both directions of this one are specified — an id in, an id and a four-byte
    /// timestamp back — but which ids exist and what each one means is not, so it is
